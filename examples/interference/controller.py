...
for index, value in enumerate(offset_values):
    ...
    timing_shell = subprocess.Popen(shlex.split('make term PORT=/dev/ttyUSB4 BOARD=remote-revb -C /home/relsas/RIOT-benpicco/examples/timing_control/'),stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,universal_newlines=True,bufsize=0)
    time.sleep(2)
    timing_shell.stdin.write("o%d" % (index))
    ...
    rx_shell = subprocess.Popen(shlex.split('make term PORT=/dev/ttyUSB3 BOARD=openmote-b'),stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,universal_newlines=True,bufsize=0)
    time.sleep(2)
    ...
    timer_thread = threading.Timer(test_duration + 10, halt_event.set)
    timer_thread.start()
    ...
    tx_shell = subprocess.Popen(shlex.split('make term PORT=/dev/ttyUSB1 BOARD=openmote-b'),stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,universal_newlines=True,bufsize=0)
    time.sleep(2)
