...
static void _set_option(at86rf215_t *dev, uint8_t option)
{
    ...
    if (superGHz) {
        at86rf215_reg_write16(dev, dev->RF->RG_CCF0L, _channel_center_freq_kHz_2400MHz(option) / 25 + 1);
    } else {
        at86rf215_reg_write16(dev, dev->RF->RG_CCF0L, _channel_center_freq_kHz_868MHz(option) / 25 + 1);
    }
